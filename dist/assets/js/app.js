...
// slider adaptive image
function init(){
 	width = document.querySelector('.main__slider').offsetWidth;
 	sliderWay.style.width = width*image.length + 'px';
 	image.forEach(item => {
 	...
 	if(width > 270){
 		image.forEach( imgSrc => {
 			imgSrc.src = "assets/img/slider/1.png"
 		})
 	} else if(width == 270){
 		image.forEach( imgSrc => {
 			imgSrc.src = "assets/img/slider/mobileSlide.png"
 		})
 	}
...
const mainInner =  document.querySelector('.main').offsetWidth

function paralax(e){
	this.querySelectorAll('.object').forEach(object => {
		let speed = object.getAttribute('data-speed');
		object.style.transform = `translateX(${e.clientX*speed/1000}px)`
	})
}

scene.addEventListener('mousemove', paralax)